...
from symbolchain.sc import Amount, Signature, Cosignature, TransferTransactionV1, AggregateCompleteTransactionV2
...
async def main() -> None:
    ...
    # ペイロードからTxの復元
    restored_tx_agg = AggregateCompleteTransactionV2.deserialize(unhexlify(payloadAgg))
    ...
    cosignB: Cosignature = account_b.cosign_transaction(restored_tx_agg)
    ...
    hash__restored_tx_agg: Hash256 = facade.hash_transaction(restored_tx_agg)

    print("===オフライン署名したトランザクションのアナウンス===")
    await await_transaction_status(
        str(hash__restored_tx_agg),
        NODE_URL,
        "confirmed"
    )
